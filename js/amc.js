...
/** B */
const B1HTML =  "<span class='tooltip' data-text='Esta UF es un Bloque (de ahí que la clave termine en B). Es una UF que tiene múltiples módulos distintos y un reto para realizarse en 5 semanas.'>3 UdC | AR1002B: Exploración en el hábitat: de la escala del elemento al territorio</span>" 
...
B2.addEventListener("click",function(){
    if(B2.classList.contains("active")){
        ...
        credsSeleccionados+=3;
        actualizarDom();
        addToList("B2","3 UdC | AR1007B: Modelación de la física en el ambiente construido: Estática")     
    };
});
...
B3.addEventListener("click",function(){
    if(B3.classList.contains("active")){
        ...
        credsSeleccionados+=3;
        actualizarDom();
        addToList("B3","3 UdC | AR1008B: Modelación de la física en el ambiente construido: Dinámica");     
    }
})

/** Materias */
// Materia 1 periodo 1 (y 2, se quita el id M1P2 de la lista de selección)
const html_M1P1_3periodos = "<span class='tooltip' data-text='Esta UF abarca 15 semanas. Procura seleccionarla primero al hacer tu horario'>3 UdC | MA1047: Razonamiento matemático en el ambiente construido</span>"
let M1P1 = document.getElementById("M1P1")
M1P1.addEventListener("click",function(){
    if(M1P1.classList.contains("active")){
        M1P1.classList.remove("active","alertBlock");
        M1P2.classList.remove("active","alertBlock");
        M1P3.classList.remove("active","alertBlock");
        credsSeleccionados-=3;
        actualizarDom();
        removeFromList("M1P1")
    } else {
        M1P1.classList.add("active","alertBlock");
        M1P2.classList.add("active","alertBlock");
        M1P3.classList.add("active","alertBlock");
        credsSeleccionados+=3;
        actualizarDom();
        addToList("M1P1",html_M1P1_3periodos)     
    }
});
